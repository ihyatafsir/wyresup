...
function initEventListeners() {
  ...
  function getWeb3Provider(preferred = null) {
    if (preferred === 'core' && typeof window.avalanche !== 'undefined') return window.avalanche;
    if (typeof window.avalanche !== 'undefined') return window.avalanche;
    if (typeof window.ethereum !== 'undefined') return window.ethereum;
    return null;
  }

  function switchPersona(mode) {
    const savedWallet = localStorage.getItem('wyresup_user_wallet');
    const isVerified = localStorage.getItem('wyresup_wallet_verified') === 'true';
    const meshIdent = JSON.parse(localStorage.getItem('wyresup_mesh_backup_identity') || 'null') || state.identity;

    const tabMesh = document.getElementById('tab-persona-mesh');
    const tabWallet = document.getElementById('tab-persona-wallet');
    const btnAvax = document.getElementById('btn-connect-wyrenet-wallet');
    ...
    if (mode === 'wallet' && savedWallet && savedWallet.startsWith('0x')) {
      localStorage.setItem('wyresup_active_persona', 'wallet');
      const shortAddr = savedWallet.substring(0, 6) + '...' + savedWallet.substring(savedWallet.length - 4);
      if (userIdEl) userIdEl.textContent = 'did:wyre:' + savedWallet.toLowerCase();
      if (userNameEl) userNameEl.textContent = (isVerified ? '🛡️ ' : '🦊 ') + shortAddr;
      if (userAvatarEl) {
        userAvatarEl.textContent = '🔺';
        userAvatarEl.style.background = isVerified ? 'linear-gradient(135deg, #38a169, #276749)' : 'linear-gradient(135deg, #e53e3e, #9b2c2c)';
      }
      if (btnAvax) {
        btnAvax.style.color = '#00f59b';
        btnAvax.style.filter = 'drop-shadow(0 0 6px #00f59b)';
        btnAvax.title = 'Active: Sovereign Web3 (Tap to toggle Anonymous Mesh)';
      }
      if (tabMesh && tabWallet) {
        tabMesh.style.background = 'transparent'; tabMesh.style.color = '#a0aec0';
        tabWallet.style.background = 'rgba(229, 62, 62, 0.2)'; tabWallet.style.color = '#fc8181';
      }
      if (state.ws && state.ws.readyState === WebSocket.OPEN) {
        state.ws.send(JSON.stringify({
          type: 'IDENTIFY',
          payload: { prefix: shortAddr, fullId: 'did:wyre:' + savedWallet.toLowerCase(), spaceId: state.currentSpaceId, channelId: state.currentChannelId }
        }));
      }
    } else {
      localStorage.setItem('wyresup_active_persona', 'mesh');
      const pfx = meshIdent?.prefix || 'peer';
      const fId = meshIdent?.fullId || (pfx + '@' + (meshIdent?.shortHash || '00000000'));
      if (userIdEl) userIdEl.textContent = fId;
      if (userNameEl) userNameEl.textContent = pfx;
      if (userAvatarEl) {
        userAvatarEl.textContent = pfx.substring(0, 2).toUpperCase();
        userAvatarEl.style.background = 'linear-gradient(135deg, #10b981, #047857)';
      }
      if (btnAvax) {
        btnAvax.style.color = '#fc8181';
        btnAvax.style.filter = 'none';
        btnAvax.title = 'Active: Anonymous Mesh (Tap to toggle Sovereign Web3)';
      }
      if (tabMesh && tabWallet) {
        tabMesh.style.background = 'rgba(0, 245, 155, 0.15)'; tabMesh.style.color = '#00f59b';
        tabWallet.style.background = 'transparent'; tabWallet.style.color = '#a0aec0';
      }
      if (state.ws && state.ws.readyState === WebSocket.OPEN) {
        state.ws.send(JSON.stringify({
          type: 'IDENTIFY',
          payload: { prefix: pfx, fullId: fId, spaceId: state.currentSpaceId, channelId: state.currentChannelId }
        }));
      }
    }
  }

  // 1-Tap 🔺 Button Trigger
  document.getElementById('btn-connect-wyrenet-wallet')?.addEventListener('click', (e) => {
    e.stopPropagation();
    const savedWallet = localStorage.getItem('wyresup_user_wallet');
    const activePersona = localStorage.getItem('wyresup_active_persona') || 'mesh';

    if (savedWallet && savedWallet.startsWith('0x')) {
      const nextMode = activePersona === 'wallet' ? 'mesh' : 'wallet';
      switchPersona(nextMode);
    } else {
      openModal('modal-wallet-connect');
      const input = document.getElementById('input-manual-wallet-addr');
      if (input && savedWallet) input.value = savedWallet;
    }
  });

  // Modal Persona Tabs
  document.getElementById('tab-persona-mesh')?.addEventListener('click', () => switchPersona('mesh'));
  document.getElementById('tab-persona-wallet')?.addEventListener('click', () => {
    const savedWallet = localStorage.getItem('wyresup_user_wallet');
    if (savedWallet && savedWallet.startsWith('0x')) {
      switchPersona('wallet');
    } else {
      alert('Please select or enter your wallet below first!');
    }
  });

  // Generic Injected Web3 Connect & Sign Flow
  async function connectInjectedWeb3(walletType = 'metamask') {
    ...
    if (!provider) {
      if (isMobile) {
        if (walletType === 'core') {
          ...
          window.location.href = 'intent://#Intent;package=me.rainbow;action=android.intent.action.VIEW;end';
          return;
        } else if (walletType === 'trust') {
          window.location.href = 'intent://#Intent;package=com.wallet.crypto.trustapp;action=android.intent.action.VIEW;end';
          return;
        } else if (walletType === 'coinbase') {
          window.location.href = 'intent://#Intent;package=org.toshi;action=android.intent.action.VIEW;end';
          return;
        }
      }
      alert('Wallet extension not detected in this browser.\n\nTip: Open WyreSup inside your mobile wallet browser or paste your 0x... address below!');
      return;
    }
    ...
    try {
      const wyreNetConfig = {
        ...
        rpcUrls: [window.location.origin + '/api/wyrenet/rpc', 'https://wyresup.com/api/wyrenet/rpc'],
        blockExplorerUrls: [window.location.origin + '/api/wyrenet/status']
      };

      try {
        await provider.request({ method: 'wallet_switchEthereumChain', params: [{ chainId: '0xCAEE' }] });
      } catch (switchErr) {
        if (switchErr.code === 4902 || (switchErr.message && switchErr.message.includes('Unrecognized chain'))) {
          await provider.request({ method: 'wallet_addEthereumChain', params: [wyreNetConfig] });
        }
      }
      ...
      if (accounts && accounts.length > 0) {
        ...
        // 1. Get Challenge
        const chalRes = await fetch('/api/wyrenet/auth/challenge/' + addr);
        ...
        try {
          signature = await provider.request({
            ...
            params: [chalData.message, addr]
          });
        } catch (sigErr1) {
          try {
            signature = await provider.request({
              method: 'personal_sign',
              params: [addr, chalData.message]
            });
          } catch (sigErr2) {
            console.warn('Personal sign note:', sigErr2);
            localStorage.setItem('wyresup_user_wallet', addr);
            localStorage.setItem('wyresup_wallet_verified', 'false');
            switchPersona('wallet');
        ...
        // 3. Verify Signature
        const verifyRes = await fetch('/api/wyrenet/auth/verify', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        localStorage.setItem('wyresup_user_wallet', addr);
        localStorage.setItem('wyresup_wallet_verified', verifyData.verified ? 'true' : 'false');
        switchPersona('wallet');
        closeModal('modal-wallet-connect');

        const balRes = await fetch('/api/wyrenet/balance/' + addr);
        const balData = await balRes.json();
        alert('🛡️ Connected to WyreNet L1!\n\nWallet: ' + addr + '\nStatus: ' + (verifyData.verified ? 'VERIFIED KEYHOLDER' : 'CONNECTED') + '\nBalance: ' + (balData.balanceWYRE || '100.0000') + ' WYRE');
      }
    } catch (err) {
      alert('Wallet notice: ' + (err.message || err));
    }
  }

  // Multi-Wallet Click Handlers
  document.getElementById('btn-wallet-metamask')?.addEventListener('click', () => connectInjectedWeb3('metamask'));
  document.getElementById('btn-wallet-core')?.addEventListener('click', () => connectInjectedWeb3('core'));
  document.getElementById('btn-wallet-rainbow')?.addEventListener('click', () => connectInjectedWeb3('rainbow'));
  document.getElementById('btn-wallet-trust')?.addEventListener('click', () => connectInjectedWeb3('trust'));
  document.getElementById('btn-wallet-coinbase')?.addEventListener('click', () => connectInjectedWeb3('coinbase'));

  // ➕ 1-Click Auto-Add WyreNet L1 Network to Wallet
  document.getElementById('btn-auto-add-network')?.addEventListener('click', async () => {
    const provider = getWeb3Provider();
    const wyreNetConfig = {
      chainId: '0xCAEE',
      chainName: 'WyreNet Sovereign L1',
      nativeCurrency: { name: 'WyreNet Token', symbol: 'WYRE', decimals: 18 },
      rpcUrls: ['https://wyresup.com/api/wyrenet/rpc'],
      blockExplorerUrls: ['https://wyresup.com/api/wyrenet/status']
    };
    ...
    if (provider) {
      try {
        await provider.request({
          ...
          params: [wyreNetConfig]
        });
        alert('✅ WyreNet Sovereign L1 Network added to your wallet successfully!');
      } catch (err) {
        alert('Network add notice: ' + (err.message || err));
      }
    } else {
      const rpcText = 'Network: WyreNet Sovereign L1\nRPC URL: https://wyresup.com/api/wyrenet/rpc\nChain ID: 51950\nSymbol: WYRE\nExplorer: https://wyresup.com/api/wyrenet/status';
      navigator.clipboard?.writeText(rpcText);
      alert('📋 WyreNet RPC details copied to clipboard!\n\nTo add manually in MetaMask:\nSettings -> Networks -> Add Network -> Custom Networks -> Paste details.');
    }
  });

  // 📋 Copy RPC Config Button
  document.getElementById('btn-copy-rpc-config')?.addEventListener('click', () => {
    const rpcText = 'Network Name: WyreNet Sovereign L1\nRPC URL: https://wyresup.com/api/wyrenet/rpc\nChain ID: 51950\nSymbol: WYRE\nExplorer: https://wyresup.com/api/wyrenet/status';
    navigator.clipboard?.writeText(rpcText);
    alert('📋 WyreNet RPC parameters copied to clipboard!');
  });

  // Manual Address Binding
  document.getElementById('btn-save-manual-wallet')?.addEventListener('click', async () => {
    const input = document.getElementById('input-manual-wallet-addr');
    const addr = input ? input.value.trim() : '';
    if (!addr || !addr.startsWith('0x') || addr.length < 10) {
      alert('Please enter a valid Ethereum / Avalanche address starting with 0x');
      return;
    }
    ...
    try {
      const balRes = await fetch('/api/wyrenet/balance/' + addr);
      const balData = await balRes.json();
      await fetch('/api/wyrenet/did/register', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ did: 'did:wyre:' + addr, address: addr })
      });
      alert('✅ Connected to WyreNet L1!\n\nAddress: ' + addr + '\nBalance: ' + (balData.balanceWYRE || '100.0000') + ' WYRE');
    } catch (err) {}
  });
