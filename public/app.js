...
function simulateBotAnswerCall(botPeerId, callType) {
  ...
  const botStream = createSyntheticStream(callType === 'video', '/cached_videos/BrPffpg9KFM.mp4');
  attachRemoteStreamToMediaElements(botStream, callType);

  const rVideo = document.getElementById('call-remote-video');
  const fallback = document.getElementById('remote-avatar-fallback');
  if (rVideo) {
    rVideo.src = '/cached_videos/BrPffpg9KFM.mp4';
    rVideo.style.display = 'block';
    rVideo.muted = false;
    rVideo.volume = 1.0;
    rVideo.play().catch(() => {});
  ...
  startCallTimer();
  document.getElementById('call-remote-status-text').textContent = 'Damascus // Yasiin Bey & Omar Souleyman (مُتَّصِل)';
  document.getElementById('call-hud-latency').textContent = '4ms (P2P DIRECT)';
}

function createSyntheticStream(withVideo = true, videoSrc = '/cached_videos/BrPffpg9KFM.mp4') {
  const AudioCtxClass = window.AudioContext || window.webkitAudioContext;
  let ctx = state.audioCtx;
  if (!ctx || ctx.state === 'closed') {
  ...
  // 1. Create or reuse stream video element
  let videoEl = document.getElementById('wyresup-synthetic-video-source');
  if (!videoEl) {
    videoEl = document.createElement('video');
    videoEl.id = 'wyresup-synthetic-video-source';
  ...
  videoEl.play().catch(e => {
    console.warn('[Synthetic Video Play Muted Fallback]:', e);
    videoEl.muted = true;
    videoEl.play().catch(() => {});
  });
  ...
  if (typeof videoEl.captureStream === 'function' || typeof videoEl.mozCaptureStream === 'function') {
    try {
      const vStream = (videoEl.captureStream || videoEl.mozCaptureStream).call(videoEl);
      const aTracks = vStream.getAudioTracks();
      if (aTracks.length > 0) audioTrack = aTracks[0];
    } catch(e) {}
  }
  ...
  if (!audioTrack) {
    try {
      if (!videoEl._sourceNode && ctx) {
        videoEl._sourceNode = ctx.createMediaElementSource(videoEl);
      }
      const dst = ctx.createMediaStreamDestination();
      const gain = ctx.createGain();
      gain.gain.setValueAtTime(0.85, ctx.currentTime);
      if (videoEl._sourceNode) {
        videoEl._sourceNode.connect(gain);
        gain.connect(dst);
        gain.connect(ctx.destination);
      }
      audioTrack = dst.stream.getAudioTracks()[0];
  ...
  // 3. Canvas with Damascus Video + Glowing Cyberpunk HUD
  const canvas = document.createElement('canvas');
  canvas.width = 1280;
  canvas.height = 720;
  const cCtx = canvas.getContext('2d');
  ...
  const animInterval = setInterval(() => {
    ...
    if (videoEl.readyState >= 2) {
      try {
        cCtx.drawImage(videoEl, 0, 0, 1280, 720);
      } catch (e) {}

      // Top Glass HUD Bar
      cCtx.fillStyle = 'rgba(4, 7, 13, 0.75)';
      cCtx.fillRect(0, 0, 1280, 80);

      // Title
      cCtx.fillStyle = '#00f59b';
      cCtx.font = 'bold 24px monospace';
      cCtx.fillText('WYRESUP // DAMASCUS (GORILLAZ FT. YASIIN BEY & OMAR SOULEYMAN) 🇸🇾', 30, 42);

      // Meta
      cCtx.font = '15px monospace';
      cCtx.fillStyle = '#00e5ff';
      cCtx.fillText('1080p P2P Cryptographic Mesh Stream · ChaCha20-Poly1305 / Opus', 30, 68);
      ...
      cCtx.fillText('LOADING YASIIN BEY DAMASCUS VIDEO STREAM...', 300, 360);
    }
  }, 1000 / 30);
  ...
  return new MediaStream([audioTrack, videoTrack].filter(Boolean));
}essage Queue (تَابُور الرَّسَائِل و صُمُود الانْقِطَاع) ---
class TaburQueue {
  static getQueue() {
    try {
      const q = localStorage.getItem("wyresup_tabur_queue");
